...
public class TestApp_partial extends Applet {

	public static void install(byte[] aArray, short sOffset, byte bLength) {
		new TestApp_partial(aArray, sOffset, bLength);
	}

	private TestApp_partial(byte[] aArray, short sOffset, byte bLength) {
	...
	public void process(APDU apdu) throws ISOException {
		...
		if (selectingApplet()) {
			byte length = JCSystem.getAID().getBytes(outBuffer, (short) 0);
